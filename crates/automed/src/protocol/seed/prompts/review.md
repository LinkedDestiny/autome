...
先读 `docs/{slug}/{slug}-task.md`，再独立复核 `docs/{slug}/{slug}.md`。

本轮产出：覆盖写 `docs/{slug}/{slug}-review.md`，逐条列出问题。**只有协议「可以要求再评审一轮的六类问题」里的六类才能提**，每条必须写明它违反的任务要求编号、设计条款或里程碑验收命令；「里程碑不可执行」这一类有一条硬判据：`### M-xx 验收契约` 三行（验收命令、预期、负向对照）缺一即不可执行，不缺就别拿「验收不够明确」说事；追溯不了的一律写进「非阻塞建议」，由裁决轮决定是否进 Backlog。没有问题时也要写出这个结论。
