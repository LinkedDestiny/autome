<!-- autome-scaffold-version: 8 -->
# Loop 协议

本文件是六个角色共同遵守的规则。Autome 在创建任务时把它整份复制到
`docs/<slug>/protocol/`，后续每一轮读的都是那份副本——所以一个任务从创建
那一刻起就固定了自己的规则版本，之后发布新版协议也影响不到它。
...
- 本轮简报 `docs/<slug>/brief/<角色>-<轮次>.md` —— Autome 生成，会话只读。
- 协议副本 `docs/<slug>/protocol/` —— 本任务固定的那一版协议，Autome 在创建
  任务时放进来。**它是本任务规则的唯一一份**，任务文件不再抄写协议正文。
...
每个里程碑在里程碑表**之外**有一个固定小节，三行：

```markdown
### M-02 验收契约

- 验收命令：`cargo test -p checkout promo`
- 预期：12 个用例通过，比上一轮基线多 3 个（优惠码上限、过期、叠加）。
- 负向对照：上限判断写成 `>=` 时，第 3、7 号用例失败。
```

三行的用处各不相同，缺一不可：**验收命令**是会话自己能跑的那条；**预期**让
「跑过了」变成可核对的数，而不是一句话；**负向对照**是唯一能说明这条命令有
鉴别力的东西——拦不住错误实现的验收，通过了也证明不了什么。

评审轮判「里程碑不可执行」有了硬判据：三行缺一即不可执行，不缺就不得拿
「验收不够明确」再要一轮。审计轮先跑负向对照，确认验收命令真的会红，再去
复验。
...
**这条规则对任何轮次都成立，不只是设计轮。** 任何一轮发现某个**已经存在的**
里程碑的验收条件需要真人，就地把它转成清单条目——不要把它挂在 `待审` 上等一个
永远不会来的证据。整个里程碑都是人工验收的，就把它拆成清单条目之后关闭它。

理由：一次真实运行把「端到端真机验收」做成了最后一个里程碑，它要真实鼠标和
麦克风，会话永远拿不到证据，于是这个里程碑谁也关不上，预算耗尽仍停在那里。
这条规则是那次运行之后加的，但它只对设计轮说了，于是同一个循环又卡了第二次：
里程碑早就立在那里，后面每一轮都照着它跑，没有人被授权去改它。
...
**没有第四种结论。** 三种都会动里程碑表：通过标 `已完成`，实现缺陷退回 `开放`，
验证缺口补强复验后照常关闭。审计轮结束时表和开始时一样，就是落在了不存在的
第四种上——Autome 会在这里停下任务等人。

最常见的那一种长这样：里程碑剩下的未取证项**全是人工验收项**（真实鼠标、
麦克风、系统弹窗、肉眼看横幅），审计既拿不到证据关闭它，又没有产品缺陷可以
退回它。这时**不得**保持 `待审`。按「验收必须是会话自己能跑的」处置：把这些项
逐条移进 `## 人工验收清单`（沿用 `H-xx` 编号，一条一行），然后按剩下那些
会话自己能跑的验收项下结论并关闭里程碑。不计 reopen，不退回实现轮。人工那几条
由用户在合并前确认。

保持 `待审` 看起来最诚实，实际上是最贵的：下一个实现轮会发现没有 `开放` 的
里程碑、无事可做，这一对会一直空转到预算耗尽。一次真实运行这样烧掉了十三轮。
